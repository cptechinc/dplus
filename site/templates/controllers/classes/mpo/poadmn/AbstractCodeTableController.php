...
use Dplus\Codes;
// Controllers
use Controllers\Mpo\Poadmn\AbstractController;
...
============================================================= */
	...
	protected static function renderJs(WireData $data) {
		...
		if ($config->twigloader->exists("code-tables/mpo/$class/.js.twig") === false) {
			return '';
		}
		return $config->twig->render("code-tables/mpo/$class/.js.twig", [$class => static::getCodeTable()]);
	}
...
============================================================= */
	public static function initHooks() {
		...
		if (static::USE_EDIT_PAGE) {
			$m->addHook('Page(pw_template=poadmn)::codeListUrl', function($event) {
				$event->return = static::url($event->arguments(0));
			});
	
			$m->addHook('Page(pw_template=poadmn)::codeAddUrl', function($event) {
				$event->return = self::codeEditUrl('new');
			});
	
			$m->addHook('Page(pw_template=poadmn)::codeEditUrl', function($event) {
