...
use Dplus\ScreenFormatters\ScreenFormatter as Formatter;
...
============================================================= */
	...
	private static function orders($data) {
		self::getData($data);
		self::pw('page')->headline = "VI: $data->vendorID Uninvoiced Purchase Orders";
...
============================================================= */
	private static function getData($data) {
		...
		if ($session->getFor('vi', 'uninvoiced-pos') > 3 && $json['vendid'] == $data->vendorID) {
			return false;
		}
		$session->setFor('vi', 'uninvoiced-pos', ($session->getFor('vi', 'uninvoiced-pos') + 1));
...
============================================================= */
	...
	private static function displayOrders($data) {
		...
		$vendor    = self::getVendor($data->vendorID);
		return $config->twig->render('vendors/vi/purchase-orders/uninvoiced/display.twig', ['vendor' => $vendor, 'json' => $json, 'formatter' => self::getFormatter(), 'docm' => $docm]);
	}
...
============================================================= */
	public static function ordersUrl($vendorID, $refreshdata = false) {
		$url = new Purl(self::viPurchaseOrdersUninvoicedUrl($vendorID));
...
============================================================= */
	private static function requestJson($vars) {
		...
		$data = ['VIUNINVOICED', "VENDID=$vars->vendorID"];
		if ($vars->shipfromID) {
