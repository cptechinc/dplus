...
============================================================= */
	...
	protected static function renderJs(WireData $data) {
		...
		if ($config->twigloader->exists("code-tables/mpm/$class/.js.twig") === false) {
			return '';
		}
		return $config->twig->render("code-tables/mpm/$class/.js.twig", [$class => static::getCodeTable()]);
	}
...
============================================================= */
	public static function initHooks() {
		...
		if (static::USE_EDIT_PAGE) {
			$m->addHook('Page(pw_template=mpm)::codeListUrl', function($event) {
				$event->return = self::url($event->arguments(0));
			});
	
			$m->addHook('Page(pw_template=mpm)::codeAddUrl', function($event) {
				$event->return = self::codeEditUrl('new');
			});
	
			$m->addHook('Page(pw_template=mpm)::codeEditUrl', function($event) {
