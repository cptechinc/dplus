<?php
	...
	if ($vendorquery->count()) {
		...
		if ($module_json->file_exists(session_id(), $page->jsoncode)) {
			...
			$module_formatter = $modules->get('ViUnreleasedPurchaseOrders');
			...
			$page->body .= $config->twig->render('vendors/vi/purchase-orders/unreleased-purchase-orders.twig', ['page' => $page, 'vendorID' => $vendorID, 'json' => $json, 'module_formatter' => $module_formatter, 'blueprint' => $module_formatter->get_tableblueprint(), 'document_management' => $document_management]);
		} else {
