	...
	if ($msa_codetables->validate_codetable($page->codetable)) {
		...
		if ($input->get->code) {
			...
			if ($module_codetable->code_exists($code)) {
				$login_group = $module_codetable->get_code($code);
			} else {
				$login_group = new SysLoginGroup();
			}

			$page->body .= $config->twig->render("code-tables/msa/$page->codetable/form.twig", ['page' => $page, 'login_group' => $login_group]);
		} else {
