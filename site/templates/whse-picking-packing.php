<?php
	...
	$config_inventory = $pages->get('/config/warehouse/inventory');
	...
	if ($input->get->ordn) {
		...
		if ($whsesession->is_orderinvalid()) { // Validate Sales Order Number
			...
			if (PickSalesOrderDetailQuery::create()->countByOrder(session_id(), $ordn) > 0) {
				...
				$page->body =  $config->twig->render('warehouse/picking/packing/picking-form.twig',    ['page' => $page, 'whsesession' => $whsesession, 'pickitem' => $pickitem, 'pickorder' => $pickorder]);
				$page->body .= $config->twig->render('warehouse/picking/packing/picked-barcodes.twig', ['page' => $page, 'picked_barcodes' => $picked_barcodes]);
				...
				$http->get($page->parent->child('template=redir')->httpUrl."?action=start-pick-pack&sessionID=".session_id());
				$page->formurl = $page->parent->child('template=redir')->url;
				$page->body = $config->twig->render('warehouse/picking/status.twig', ['page' => $page, 'whsesession' => $whsesession]);
				$page->body .= '<div class="form-group"></div>';
				$page->body .= $config->twig->render('warehouse/picking/sales-order-form.twig', ['page' => $page]);
